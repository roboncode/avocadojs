...
describe('orango subdocs', function() {
  ...
  describe('with _key', function() {
    it('be valid', async function() {
      ...
      let aql = await test.save().toAQL()
      expect(aql).to.equal(
        'FOR $test IN tests FILTER ($test.`_key` == "1") LIMIT 1 UPDATE $test WITH {} IN tests COLLECT WITH COUNT INTO modified RETURN { modified }'
      )
    })
  })
  ...
  describe('defaults', function() {
    it('to have defaults', async function() {
      ...
      expect(aql).to.equal(
        'FOR $test IN tests FILTER ($test.`_key` == "1") LIMIT 1 UPDATE $test WITH {"name":"test"} IN tests COLLECT WITH COUNT INTO modified RETURN { modified }'
      )
    })
  })
  ...
  describe('array of strings', function() {
    it('to have array of strings', async function() {
      ...
      let aql = await test.save().toAQL()
      expect(aql).to.equal(
        'FOR $test IN tests FILTER ($test.`_key` == "1") LIMIT 1 UPDATE $test WITH {"tags":["foo","bar"]} IN tests COLLECT WITH COUNT INTO modified RETURN { modified }'
      )
    })
  })
  ...
  describe('array of objects with no $id', function() {
    it('to have array of objects with no $id', async function() {
      ...
      let aql = await test.save().toAQL()
      expect(aql).to.equal(
        'FOR $test IN tests FILTER ($test.`_key` == "1") LIMIT 1 UPDATE $test WITH {"comments":[{"text":"test"}]} IN tests COLLECT WITH COUNT INTO modified RETURN { modified }'
      )
    })
  })
  ...
  describe('array of objects with $id', function() {
    it('to have array of objects with $id of "test"', async function() {
      ...
      let aql = await test.save().toAQL()
      expect(aql).to.equal(
        'FOR $test IN tests FILTER ($test.`_key` == "1") LIMIT 1 UPDATE $test WITH {"comments":[{"$id":"test","text":"test"}]} IN tests COLLECT WITH COUNT INTO modified RETURN { modified }'
      )
    })
  })
  ...
  describe('array pushing primitive values', function() {
    it('to have array of primitive values', async function() {
      ...
      let aql = await test.save().toAQL()
      expect(aql).to.equal(
        'FOR $test IN tests FILTER ($test.`_key` == "1") LIMIT 1 LET tags = APPEND($test.tags, ["foo","bar"]) UPDATE $test WITH {"tags":tags} IN tests COLLECT WITH COUNT INTO modified RETURN { modified }'
      )
    })
  })
  ...
  describe('array pulling objects', function() {
    it('to have array of primitive values with $id', async function() {
      ...
      let aql = await test.save().toAQL()
      expect(aql).to.equal(
        'FOR $test IN tests FILTER ($test.`_key` == "1") LIMIT 1 LET comments = MINUS($test.comments, ( FOR item IN $test.comments || [] FOR id IN ["test"] FILTER item.$id == id RETURN item)) UPDATE $test WITH {"comments":comments} IN tests COLLECT WITH COUNT INTO modified RETURN { modified }'
      )
    })
  })
  ...
  describe('array pulling objects with $pull', function() {
    it('to minus', async function() {
      ...
      let aql = await test.save().toAQL()
      expect(aql).to.equal(
        'FOR $test IN tests FILTER ($test.`_key` == "1") LIMIT 1 LET comments = MINUS($test.comments, ( FOR $test_comment IN $test.comments FILTER (($test_comment.$id == "test") OR ($test_comment.`user` == "@test")) RETURN $test_comment)) UPDATE $test WITH {"comments":comments} IN tests COLLECT WITH COUNT INTO modified RETURN { modified }'
      )
    })
  })
  ...
  describe('array pulling objects with $pull', function() {
    it('to minus', async function() {
      ...
      let aql = await test.save().toAQL()
      expect(aql).to.equal(
        'FOR $test IN tests FILTER ($test.`_key` == "1") LIMIT 1 LET comments = MINUS($test.comments, ( FOR item IN $test.comments || [] FOR id IN ["foo","bar"] FILTER item.$id == id RETURN item)) UPDATE $test WITH {"comments":comments} IN tests COLLECT WITH COUNT INTO modified RETURN { modified }'
      )
    })
  })
})
