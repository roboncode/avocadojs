...
class ORM {
  ...
  _parseSubdocs(arrayPaths, data) {
    for (let i = 0; i < arrayPaths.length; i++) {
      ...
      if (items && (items.isOverridden || items.$push || items.$pull)) {
        ...
        try {
          if (items.$pull) {
            this._subdocsOptions.pullIds[PATH_DOT] = items.$pull
          }
        } catch (e) {}
  ...
  _createAQLUpdate() {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      this.aqlSegments.push(`UPDATE ${DOC_VAR} WITH ${SPECIAL_TAG}`)
  ...
  _createAQLRemove() {
    this.aqlSegments.push('REMOVE ' + DOC_VAR)
    this.aqlSegments.push('IN ' + this._collection.name)
  }
