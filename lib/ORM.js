...
const { ACTIONS, EVENTS, RETURN, TANG } = require('./consts')
require('colors')
...
class ORM {
  ...
  _createAQLReturn(distinct = false) {
    ...
    if (this._return !== RETURN.VAR && this._return !== RETURN.MODEL) {
  ...
  _find() {
    if(this._return !== RETURN.VAR && this._return !== RETURN.MODEL) {
      throw new Error(`Return value must be "${RETURN.VAR}" or "${RETURN.MODEL}"`)
    }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (result && (this._return === RETURN.VAR || this._return === RETURN.MODEL)) {
