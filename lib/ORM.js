...
class ORM {
  ...
  _createAQLSubdocs(arrayPaths, data) {
    for (let i = 0; i < arrayPaths.length; i++) {
      ...
      const arr = lodashGet(data, PATH_DOT)
      const removeIds = jsonStringify(arr.removeIds || [])
      ...
      let subdocsJSON = jsonStringify(subdocs)

      let aql =
        this._separator +
        `LET ${PATH_VAR} = APPEND(MINUS(${DOC_VAR}.${PATH_DOT}, (
