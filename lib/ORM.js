...
const { ACTIONS, AQL, ERRORS, EVENTS, RETURN, TANG } = require('./consts')
require('colors')
...
class ORM {
  ...
  _createAQLReturn(distinct = false) {
    if (this._return === RETURN.NEW_DOC || this._return === RETURN.NEW_MODEL) {
      return this.aqlSegments.push('RETURN NEW')
    }

    if (this._return === RETURN.OLD_DOC || this._return === RETURN.OLD_MODEL) {
      return this.aqlSegments.push('RETURN OLD')
    }

    if (this._return === RETURN.NEW_OLD_DOC || this._return === RETURN.NEW_OLD_MODEL) {
  ...
  _insert() {
    this._model.emit(EVENTS.CREATE, {
      model: this._model,
      data: this._data,
      orm: this
    })
    ...
    promise.then(
      (data) => {
        if (data) {
          this._model.emit(EVENTS.CREATED, {
            model: this._model,
            data,
            orm: this
          })
  ...
  _update() {
    this._model.emit(EVENTS.UPDATE, {
      model: this._model,
      data: this._data,
      orm: this
    })
      ...
      try {
        ...
        if (
          ...
          if (result) {
            ...
            if (this._return === RETURN.NEW_OLD_DOC || this._return === RETURN.NEW_OLD_MODEL) {
              return resolve({ new: result[0], old: result[1] })
            }
            return resolve(result)
          }
        }
    ...
    promise.then((data) => {
      this._model.emit(EVENTS.UPDATED, {
        model: this._model,
        data,
        orm: this
      })
  ...
  _delete() {
    this._model.emit(EVENTS.DELETE, {
      model: this._model,
      orm: this
    })
    ...
    promise.then((docs) => {
      this._model.emit(EVENTS.DELETED, {
        model: this._model,
        data: docs,
        orm: this
      })
