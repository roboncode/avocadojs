...
class ORM {
  ...
  _createAQLReturn(distinct = false) {
    if (this._return === RETURN.NEW_DOC) {
      return this.aqlSegments.push('RETURN NEW')
    }

    if (this._return === RETURN.OLD_DOC) {
      return this.aqlSegments.push('RETURN OLD')
    }

    if (this._return === RETURN.NEW_OLD_DOC) {
  ...
  _insert() {
    ...
    const promise = new Promise(async (resolve, reject) => {
      try {
        ...
        if (data) {
          let doc = await this._collection.save(
            data
          )
  ...
  async _createUpdateQuery() {
    let returnVal = this._return === RETURN.NEW_DOC || this._return === RETURN.OLD_DOC || this._return === RETURN.NEW_OLD_DOC
    if (!returnVal) {
