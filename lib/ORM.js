...
const { ACTIONS, AQL, HOOKS, RETURN, TANG } = require('./consts')
require('colors')
...
class ORM {
  ...
  async toAQL(prettify = false) {
    ...
    if (this._action === ACTIONS.DELETE || this._action === ACTIONS.UNLINK) {
  ...
  exec() {
    ...
    if (this._action === ACTIONS.LINK) {
      return this._link()
    }

    if (this._action === ACTIONS.UNLINK) {
      return this._unlink()
    }
  ...
  _createAQLReturn(distinct = false) {
    ...
    if (this._action === ACTIONS.UPDATE && isDoc(this._return)) {
  ...
  _unlink() {
    console.log('_UNLINK CALLED'.bgGreen)
