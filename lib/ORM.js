...
const { sortToAQL, filterToAQL, setDefaultsToNull, arrayOverride, createUniqueId } = require('./helpers')
...
class ORM {
  ...
  */
  ...
  populate2(filter) {
    console.log('pop2'.bgMagenta, filter)
    return this
  }
  ...
  _createAQLReturnCount(prop = METHODS.COUNT) {
    this._one = true
    this.aqlSegments.push(`COLLECT WITH COUNT INTO ${prop} RETURN { ${prop} }`)
  }
  ...
  _createAQLForInBound() {
    this.aqlSegments.push(
      ...
      this.getDocName(),
      'IN',
      this._filter.inbound ? 'INBOUND' : 'OUTBOUND',
      `"${this._filter.id}"`,
  ...
  _createAQLFilter() {
    if (Object.keys(this._filter).length) {
      this.aqlSegments.push(
        ...
        filterToAQL(this._filter, this._parentDoc || this.getDocName())
      )
    }
  }
  ...
  async _createAQLSubdocs(arrayPaths, data) {
    for (let i = 0; i < arrayPaths.length; i++) {
      ...
      let pullFilter = this._subdocsOptions.pulls[PATH_DOT]
      let items = lodashGet(data, PATH_DOT) || []
      try {
        if (items.$push) {
          isNew = true
      ...
      if (pullFilter) {
        ...
        if (pullFilter instanceof Array) {
          let ids = JSON.stringify(pullFilter)
          aql =
