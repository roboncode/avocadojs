...
class ORM {
  ...
  _createAQLSubdocs(arrayPaths, data) {
    for (let i = 0; i < arrayPaths.length; i++) {
      ...
      if (items && items.length) {
        ...
        if (pullIds !== '[]') {
          aql = `LET ${PATH_VAR} = APPEND(MINUS(${DOC_VAR}.${PATH_DOT}, (
          ...
          aql = `LET ${PATH_VAR} = APPEND(${DOC_VAR}.${PATH_DOT}, ${subdocsJSON})`
        }

        this.aqlSegments.push(this._separator, aql, this._separator)
      } else {
        let aql = `LET ${PATH_VAR} = MINUS(${DOC_VAR}.${PATH_DOT}, (
          FOR item IN ${DOC_VAR}.${PATH_DOT} || []
              FOR id IN ${pullIds}
                  FILTER item.$id == id
              RETURN item
  ...
  _createInsertQuery() {
    return 'INSERT QUERY CANNOT BE DISPLAYED'
  }
