...
class ORM {
  ...
  get _isCustomReturn() {
    return !(
      this._return === RETURN.ONE ||
      this._return === RETURN.MANY ||
  ...
  _createAQLReturn(distinct = false) {
    if (this._return === RETURN.NEW) {
      return this.aqlSegments.push('RETURN NEW')
    }

    if (this._return === RETURN.OLD) {
      return this.aqlSegments.push('RETURN OLD')
    }

    if (this._return === RETURN.NEW_OLD) {
  ...
  async _createAQLSets() {
    for (let i = 0; i < this._populates.length; i++) {
      ...
      if (item.value instanceof ORM) {
        ...
        let first = item.value._return === RETURN.ONE || item.value._return === RETURN.MODEL ? 'FIRST' : ''
  ...
  _find() {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (result && (this._return === RETURN.ONE || this._return === RETURN.MODEL)) {
  ...
  async _createUpdateQuery() {
    let returnVal = this._return === RETURN.NEW || this._return === RETURN.OLD || this._return === RETURN.NEW_OLD
    if (!returnVal) {
  ...
  _update() {
    ...
    const promise = new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          this._return === RETURN.ONE ||
          this._return === RETURN.NEW ||
          this._return === RETURN.OLD ||
          this._return === RETURN.NEW_OLD ||
          this._return === RETURN.MODEL
        ) {
  ...
  _delete() {
    ...
    const promise = new Promise(async (resolve, reject) => {
      try {
        ...
        let cursor = await this._connection.db.query(query)
        if (this._return === RETURN.ONE || this._return === RETURN.MODEL) {
