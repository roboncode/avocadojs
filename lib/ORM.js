...
class ORM {
  ...
  getDoc() {
    return this._doc || pluralize.singular(this._model.collectionName)
  }
  ...
  join(property, value, options = { merge: false }) {
    if (value instanceof ORM) {
      value.parentDoc(this.getDoc())
    }
    this._joins.push({
      property,
      value,
  ...
  _compileAQLSegments() {
    let query = this.aqlSegments.join(' ').replace(EXPR, this.getDoc() + '.$1')
    query = query.replace(/'(@@doc)(.\w+)'/g, /*this.getDoc()*/ 'doc' + '$2')

    if(query.indexOf('@@parent') !== -1) {
      // query = query.replace(/'(.*@@.*)'/gi, "$1")
      // query = query.split('@@parent').join(this._parentDoc)
      // query = query.replace(/(\w+\/)([\w\.\s|"']+)/gi, "CONCAT('$1', $2)")
  ...
  _createAQLForIn() {
    this.aqlSegments.push('FOR', this.getDoc(), 'IN', this._collection.name)
  }
  ...
  _createAQLFilter() {
    if (Object.keys(this._criteria).length) {
      this.aqlSegments.push(
        ...
        criteriaBuilder(this._criteria, this._parentDoc || this.getDoc())
      )
    }
  }
  ...
  _createAQLSort() {
    if (this._sort) {
      this.aqlSegments.push(
        this._separator + 'SORT ' + sortToAQL(this._sort, this.getDoc())
      )
    }
  }
  ...
  //       variable = _prop
  //       this.aqlSegments.push(
  //         `LET ${variable} = DOCUMENT(CONCAT('${dm.collectionName}/', doc.${
  //           populate.property
  //         }))`
  //       )
  //     }
  ...
  //       lodashSet(mergeItem, populate.property, `KEEP(${_prop}, ${keepProps})`)
  //     } else {
  ...
  //     .join('')
  //   this.aqlSegments.push(
  //     this._separator + `RETURN MERGE(${this.getDoc()}, ${mergeItem})`
  //   )
  // }
  ...
  _createAQLReturn(distinct = false) {
    ...
    for (let i = 0; i < this._joins.length; i++) {
      if (this._joins[i].options.merge) {
        props.push(this._joins[i].property)
      } else {
        props.push('{' + this._joins[i].property + '}')
      }
    }
    ...
    let doc = this._select
      ? 'KEEP(' + this.getDoc() + ', "' + this._select.split(' ').join('","') + '")'
      : this.getDoc()
    if (props.length) {
      this.aqlSegments.push(
    ...
    for (let i = 0; i < arrayPaths.length; i++) {
      ...
      if (pullCriteria) {
        ...
        if (pullCriteria instanceof Array) {
          ...
          aql =
            `MINUS(${this.getDoc()}.${PATH_DOT}, ( ` +
            this._separator +
            `FOR item IN ${this.getDoc()}.${PATH_DOT} || [] ` +
            this._separator +
            `FOR id IN ${ids} ` +
            this._separator +
            `FILTER item.$id == id ` +
  ...
  _createAQLUpdate() {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      this.aqlSegments.push(`UPDATE ${this.getDoc()} WITH ${SPECIAL_TAG}`)
  ...
  _createAQLRemove() {
    this.aqlSegments.push('REMOVE ' + this.getDoc())
    this.aqlSegments.push('IN ' + this._collection.name)
  }
  ...
  _createAQLVar() {
    // this.getDoc() = createUniqueId('doc_')
    this._return = this.getDoc()
    this.aqlSegments.push(`LET ${this.getDoc()} =`)
  }
  ...
  _createAQLDocument() {
    ...
    // if (key.indexOf('@@') === -1) {
    //   documentId = this._model.documentId(key)
    // }
    // if (this._select) {
    //   this.aqlSegments.push('KEEP(')
    // }
    this.aqlSegments.push(`DOCUMENT('${documentId}')`)

    // if (this._select) {
    //   let keepProps = "'" + this._select.split(' ').join("', '") + "'"
    //   this.aqlSegments.push(',' + keepProps + ')')
    // }
  }

  async _createAQLSets() {
    for (let i = 0; i < this._joins.length; i++) {
      let item = this._joins[i]
      if (item.value instanceof ORM) {
        let aql = await item.value.toAQL()
        let first = aql.indexOf('DOCUMENT') === -1 ? '' : 'FIRST'
