...
const { ACTIONS, EVENTS } = require('./consts')
require('colors')
...
class ORM {
  ...
  _compileAQLSegments() {
    ...
    if ((this._action === ACTIONS.DOCUMENT || this._action === ACTIONS.FIND || this._action === ACTIONS.FIND_EDGE) && !this._parentDoc) {
  ...
  _createAQLReturn(distinct = false) {
    console.log('_return', this._return)
    if (this._return) {
  ...
  _find() {
    console.log('FIND'.bgRed, this._options)
    return new Promise(async (resolve, reject) => {
      try {
        const query = this._action === ACTIONS.DOCUMENT ? await this._createDocumentQuery() : await this._createFindQuery()
        ...
        if (!this._options.returnModel) {
          builder.convertTo(this._model).toObject({
            skipValidation: true,
            computed: this._computed,
            noDefaults: !this._defaults
            // unknownProps: this._schemaOptions.strict ? 'strip' : 'allow'
