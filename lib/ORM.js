...
class ORM {
  ...
  set(property, value, merge = false) {
    if (value instanceof ORM) {
      value.parentDoc(this._doc)
    }
    this._sets.push({
      property,
      value,
  ...
  _compileAQLSegments() {
    ...
    query = query.replace(/'(@@doc)(.\w+)'/g, /*this._doc*/ 'doc' + '$2')

    if(!this._parentDoc) {
      query = query.replace(/'(\w+\/)@@parent(\.\w+)'/g, `CONCAT('$1',${this._doc}$2)`)
    }
  ...
  _createAQLFilter() {
    if (Object.keys(this._criteria).length) {
      this.aqlSegments.push(
        ...
        criteriaBuilder(this._criteria, this._parentDoc || this._doc)
      )
    }
  }
  ...
  _createAQLReturn(distinct = false) {
    ...
    for (let i = 0; i < this._sets.length; i++) {
      if (this._sets[i].merge) {
        props.push(this._sets[i].property)
      } else {
        props.push('{' + this._sets[i].property + '}')
      }
    }

    let doc = this._select ? 'KEEP(' + this._doc + ', "' + this._select.split(' ').join('","') + '")' : this._doc
    if (props.length) {
  ...
  _createAQLVar() {
    // this._doc = createUniqueId('doc_')
    this._return = this._doc
    this.aqlSegments.push(`LET ${this._doc} =`)
  }
  ...
  async _createAQLSets() {
    for (let i = 0; i < this._sets.length; i++) {
      let item = this._sets[i]
      if (item.value instanceof ORM) {
        let aql = await item.value.toAQL()
        let tail = aql.indexOf('DOCUMENT') === -1 ? '' : '[0]'
        this.aqlSegments.push(`LET ${item.property} = (${aql})` + tail)
      } else {
        this.aqlSegments.push(
