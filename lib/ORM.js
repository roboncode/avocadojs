...
class ORM {
  ...
  document(name, Model, id) {
    this._documents[name] = {
      Model,
      id
    }
    return this
  ...
  _createAQLDocuments() {
    for (let name in this._documents) {
      if (this._documents.hasOwnProperty(name)) {
  ...
  // _createAQLDocumentsFromPopulates() {
  //   for (let i = 0; i < this._populates.length; i++) {
  //     let populate = this._populates[i]
  //     let prop = populate.property
  //     let _prop = prop.split('.').join('_')
  //     let documentId = populate.Model.documentId(populate.options.id)
  //     this.aqlSegments.push(`LET ${_prop} = DOCUMENT('${documentId}') `)
  //   }
  // }
  ...
  _createAQLLimit() {
    if (this._offset || this._limit) {
      ...
      this.aqlSegments.push(
        this._separator + 'LIMIT ' + offset + ',' + limit
      )
    }
  }
  ...
  _createAQLPopulates() {
    ...
    for (let i = 0; i < this._populates.length; i++) {
      ...
      if (populate.options.select) {
        ...
        if (typeof dm !== 'string') {
          this.aqlSegments.push(
            `LET ${_prop} = DOCUMENT(CONCAT('${
              dm.collectionName
            }/', doc.${populate.property}))`
          )
        }
        if (populate.options.select) {
          let keepProps =
    ...
    mergeItem = JSON.stringify(mergeItem)
      ...
      .join('')
    this.aqlSegments.push(
      this._separator + `RETURN MERGE(${this._variable}, ${mergeItem})`
    )
  }
  ...
  _createFindQuery() {
    if (this._query) {
      ...
      // this._createAQLDocumentsFromPopulates()
    ...
    if (this._populates.length) {
      for (let i = 0; i < this._populates.length; i++) {
        ...
        builder.intercept(async doc => {
          ...
          let dm = populate.documentOrModel
          let Model = typeof dm === 'string' ? this._documents[dm].Model : dm
