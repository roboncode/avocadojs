...
const { ACTIONS, AQL, ERRORS, HOOKS, RETURN, TANG } = require('./consts')
require('colors')
...
class ORM {
  ...
  get _isCustomReturn() {
    return !(
      this._return === RETURN.DOC ||
      this._return === RETURN.DOCS ||
      this._return === RETURN.MODEL ||
      this._return === RETURN.MODELS
    )
  }
  ...
  async toAQL(prettify = false) {
    ...
    if (this._action === ACTIONS.INSERT) {
      return this._createInsertQuery()
    }

    if (this._action === ACTIONS.DOCUMENT) {
      return this._createDocumentQuery()
    }

    if (this._action === ACTIONS.FIND) {
      return await this._createFindQuery()
    }

    if (this._action === ACTIONS.FIND_EDGE) {
      return this._createEdgeQuery()
    }

    if (this._action === ACTIONS.UPDATE) {
      return await this._createUpdateQuery()
    }

    if (this._action === ACTIONS.DELETE) {
      return this._createDeleteQuery()
    }

    if (this._action === ACTIONS.COUNT) {
  ...
  exec() {
    ...
    if (this._action === ACTIONS.DOCUMENT || this._action === ACTIONS.FIND) {
      return this._find()
    }
  ...
  _compileAQLSegments() {
    ...
    if (
      (this._action === ACTIONS.DOCUMENT || this._action === ACTIONS.FIND || this._action === ACTIONS.FIND_EDGE) &&
      !this._parentDoc
    ) {
      query = query.replace(/([\w\$]+\/)(\$)([\$\w\s|".]+)/gi, "CONCAT('$1',$2$3)")
    }

    return query
  }

  _createAQLInitCount(prop = ACTIONS.COUNT) {
    this.aqlSegments.push(`LET ${prop} = COUNT(`)
  }

  _createAQLReturnCount(prop = ACTIONS.COUNT) {
  ...
  _createAQLReturn(distinct = false) {
    if (
      this._return === RETURN.NEW_DOC ||
      this._return === RETURN.NEW_MODEL ||
      this._return === RETURN.NEW_DOCS ||
      this._return === RETURN.NEW_MODELS
    ) {
      return this.aqlSegments.push('RETURN NEW')
    }

    if (
      this._return === RETURN.OLD_DOC ||
      this._return === RETURN.OLD_MODEL ||
      this._return === RETURN.OLD_DOCS ||
      this._return === RETURN.OLD_MODELS
    ) {
      return this.aqlSegments.push('RETURN OLD')
    }

    if (
      this._return === RETURN.NEW_OLD_DOC ||
      this._return === RETURN.NEW_OLD_MODEL ||
      this._return === RETURN.NEW_OLD_DOCS ||
      this._return === RETURN.NEW_OLD_MODELS
    ) {
      return this.aqlSegments.push('RETURN { old: OLD, new: NEW }')
    }

    if (this._isCustomReturn) {
  ...
  async _createAQLSets() {
    for (let i = 0; i < this._populates.length; i++) {
      ...
      if (item.value instanceof ORM) {
        ...
        let first = item.value._return === RETURN.DOC || item.value._return === RETURN.MODEL ? 'FIRST' : ''
        this.aqlSegments.push(`LET ${item.property} = ${first}(${aql})`)
      } else {
        this.aqlSegments.push(`LET ${item.property} = ${JSON.stringify(item.value)}`)
      }
    }
  }
  ...
  _createCountQuery() {
    this._createAQLInitCount(ACTIONS.COUNT)
    if (this._query) {
      this._createAQLCustom()
    } else {
      this._createAQLForIn()
    ...
    this._createAQLSort()
    this._createAQLLimit()
    this._createAQLReturnCount(ACTIONS.COUNT)
  ...
  _find() {
    if (this._return && this._isCustomReturn) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const query =
          this._action === ACTIONS.DOCUMENT ? await this._createDocumentQuery() : await this._createFindQuery()
        ...
        if (result && (this._return === RETURN.DOC || this._return === RETURN.MODEL)) {
  ...
  async _createUpdateQuery() {
    ...
    //   case RETURN.NEW_DOC:
    //   case RETURN.OLD_DOC:
    //   case RETURN.NEW_OLD_DOC:
    //   case RETURN.NEW_MODEL:
    //   case RETURN.OLD_MODEL:
    //   case RETURN.NEW_OLD_MODEL:
    //   case RETURN.NEW_DOCS:
    //   case RETURN.OLD_DOCS:
    //   case RETURN.NEW_OLD_DOCS:
    //   case RETURN.NEW_MODELS:
    //   case RETURN.OLD_MODELS:
    //   case RETURN.NEW_OLD_MODELS:
    //     returnVal = true
    // }
    if (this._return) {
      returnVal = true
  ...
  _update() {
    ...
    const promise = new Promise(async (resolve, reject) => {
      try {
        ...
        let cursor = await this._connection.db.query(query)
        let results
        console.log('TEST #1'.bgRed, this._return)
        if (
          this._return === RETURN.NEW_DOC ||
          this._return === RETURN.OLD_DOC ||
          this._return === RETURN.NEW_OLD_DOC ||
          ...
          console.log('TEST #2'.bgGreen, results)
          await asyncForEach(results, async (result, index, list) => {
            ...
            if (
              ...
              console.log('YOU ARE HERE AGAIN'.bgMagenta, this._return, this._computed)
              builder.toObject({
                skipValidation: true,
                computed: this._computed,
                noDefaults: !this._defaults
            ...
            console.log('RESULT'.bgWhite, result)
            if (
              this._return === RETURN.NEW_OLD_DOC ||
              this._return === RETURN.NEW_OLD_MODEL ||
              this._return === RETURN.NEW_OLD_DOCS ||
  ...
  _delete() {
    ...
    const promise = new Promise(async (resolve, reject) => {
      try {
        ...
        let cursor = await this._connection.db.query(query)
        if (this._return === RETURN.DOC || this._return === RETURN.MODEL) {
