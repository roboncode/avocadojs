...
class Schema {
  ...
  parseDefaults(json) {
    ...
    this.updateDefaults = {}
    for (let n in json) {
      if (json[n].onInsert || json[n].onUpdate) {
        this.insertDefaults[n] = json[n].onInsert
        this.updateDefaults[n] = json[n].onUpdate
        delete json[n].onInsert
        delete json[n].onUpdate
      }
    }
  }
