...
class QueryToAQL {
  ...
  async generate(query, formatted = false) {
    ...
    let q = await this.validate(Query, query)
  ...
  async validate(Model, data, options = {}) {
    let result = await Builder.getInstance()
      .data(data)
      .convertTo(Model)
      .toObject({
        noDefaults: options.withDefaults !== true,
  ...
  async parseOperations(aql, query, { doc, col }) {
    if (query.method === OPERATIONS.UPDATE) {
      ...
      const ModelCls = this.orango.model(query.model)
      query.data.insert = await this.validate(ModelCls, query.data.insert, {
        withDefaults: query.withDefaults
      })
      query.data.update = await this.validate(ModelCls, query.data.update)
  ...
  async parseInsert(query) {
    const ModelCls = this.orango.model(query.model)
    query.data = await this.validate(ModelCls, query.data, query)
  }

  async parseUpsert(query) {
    const ModelCls = this.orango.model(query.model)
    query.data.insert = await this.validate(ModelCls, query.data.insert, query)
    query.data.update = await this.validate(ModelCls, query.data.update)
  }

  async parseImport(query) {
    const ModelCls = this.orango.model(query.model)
    query.data.data = await this.validate(ModelCls, query.data.data, query)
  }
  ...
  async parseQuery(query) {
    const ModelCls = this.orango.model(query.model)
    const col = ModelCls.getCollectionName()
