...
class QueryToAQL {
  ...
  async generate(query, formatted = false) {
    ...
    let q = await this.validate(null, this.QueryModel, query)
  ...
  async validate(operation, Model, data, options = {}) {
    let result = await builder()
      .data(data)
      .convertTo(Model)
      .intercept(async model => {
        // console.log('===>'.bgRed, Model.schema.insertDefaults)
        let d = {}
        console.log('OPS'.bgRed, operation)
  ...
  async parseOperations(aql, query, { doc, col }) {
    if (query.method === OPERATIONS.UPDATE) {
      ...
      let data = await this.validate(OPERATIONS.UPDATE, ModelCls, query.data)
      ...
      const ModelCls = this.orango.model(query.model)
      let data = AQB(
        await this.validate(OPERATIONS.UPDATE, ModelCls, query.data)
      )
      aql = aql
        .replace(doc)
        .with(data)
