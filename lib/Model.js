...
const { AQL, ERRORS, HOOKS, METHODS, RELATION, RETURN } = require('./consts')
...
class OrangoModel extends Model {
  ...
  static hasMany(modelName, options = {}) {
    ...
    if(!options.as) {
      let name = pluralize(modelName)
      options.as = name.charAt(0).toLowerCase() + name.slice(1)
    }
    options.has = RELATION.MANY
    this._hasMany[modelName] = options
    return this
  }
  ...
  static belongsTo(modelName, options = {}) {
    ...
    if (!options.as) {
      let name = pluralize.singular(modelName)
      options.as = name.charAt(0).toLowerCase() + name.slice(1)
    }
    options.has = RELATION.ONE
    this._belongsTo[modelName] = options
    return this
  }

  static getRelation(modelName) {
