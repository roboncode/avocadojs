...
class OrangoModel extends Model {
  ...
  static _createCollection() {
    ...
    if (!this._collection) {
      if (this.schema.isEdge) {
        this._collection = orango.createEdgeCollection(this.collectionName)
      } else {
        this._collection = orango.createCollection(
          this.collectionName,
          this.schema.options.indexes
        )
      }
  ...
  static on(event) {
    ...
    if (event === 'ready') {
      if (!self._events[event]) {
        self._events[event] = {
          then(resolve) {
            return self._createCollection().then(() => {
              return resolve(self)
            })
          }
        }
  ...
  static importMany(docs, truncate = false) {
    return this.on('ready').then(async () => {
      let collection = this.getCollection()

      if (truncate) {
        await collection.truncate()
      }
      return await collection.import(docs)
    })
  }
  ...
  save(options = {}) {
    if (this.constructor.orango.connection.connected) {
      ...
      return new Promise(async resolve => {
        this.constructor.orango.connection.events.on('connected', async () => {
          await this.constructor._createCollection()
          let result = await this.save(options)
          resolve(result)
        })
      })
    }
