  ...
  static _createCollection() {
    ...
    return new Promise(async resolve => {
      if (this.schema.options.edge) {
        await orango.createEdgeCollection(this.collectionName)
      } else {
        await orango.createCollection(
          this.collectionName,
  ...
  static on(event) {
    ...
    if (event === 'ready') {
      if (!self._events[event]) {
        self._events[event] = {
          then(resolve) {
            if (self.orango.connection.connected) {
              return self._createCollection().then(() => {
                return resolve(self)
              })
            }
            self.orango.connection.events.on('connected', () => {
              return self._createCollection().then(() => {
                return resolve(self)
              })
            })
