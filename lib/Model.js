...
class OrangoModel extends Model {
  ...
  static _qb(method, data) {
    ...
    // this.orango.checkConnected()
    // return new Promise(async resolve => {
    //   let result = await this.collection.truncate()
  ...
  static unlink(from, to) {
    ...
    if (from) {
      from = this.orango.model(schema.options.from).getKey(from)
    } else {
      from = undefined
    }

    if (to) {
      to = this.orango.model(schema.options.to).getKey(to)
    } else {
      to = undefined
    }
