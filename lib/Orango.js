...
class Orango {
  ...
  connect(name = '_system', {
    ...
    return this.connection.connect(name, {
      url,
      username,
      password
    })
  ...
  async createDatabase(name, users = []) {
    ...
    this.events.emit(CONSTS.EVENTS.DATABASE_CREATED, name)
    EventDispatcher.getInstance(this.$instanceName).emit('database:created', name)
  }
  ...
  async dropDatabase(name) {
    ...
    if (this.connection && this.connection.connected) {
      ...
      this.events.emit(CONSTS.EVENTS.DATABASE_DROPPED, name)
      EventDispatcher.getInstance(this.$instanceName).emit('database:dropped', name)
    }
  }
  ...
  async _connectionHandler(model) {
    ...
    this.events.emit(CONSTS.EVENTS.MODEL_READY, model)
  }
