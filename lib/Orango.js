...
class Orango extends EventEmitter {
  ...
  async createDatabase(name, users = []) {
    ...
    this.emit(CONSTS.EVENTS.DATABASE_CREATED, name)
    EventDispatcher.getInstance(this.$instanceName).emit(CONSTS.EVENTS.DATABASE_CREATED, name)
  }
  ...
  async dropDatabase(name) {
    ...
    if (this.connection && this.connection.connected) {
      ...
      this.emit(CONSTS.EVENTS.DATABASE_DROPPED, name)
      EventDispatcher.getInstance(this.$instanceName).emit(CONSTS.EVENTS.DATABASE_DROPPED, name)
    }
  }
  ...
  async _connectionHandler(model) {
    if (this.connectionHandlers.indexOf(model.collectionName) === -1) {
      ...
      model.emit(CONSTS.EVENTS.READY, model)
    }
  }
  ...
  model(name, schema, collectionName = '') {
    ...
    if (!model) {
      throw new Error(CONSTS.ERRORS.MODEL_NOT_FOUND.split('{name}').join(name))
    }
    ...
    setImmediate(async () => {
      if (!isWaiting) {
        ...
        if (this.connection.connected) {
          ...
          this.connection.once(CONSTS.EVENTS.CONNECTED, () => {
            if (!model.then) {
