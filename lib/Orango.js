...
class Orango extends EventEmitter {
  ...
  async _createCollection(model) {
    if (this.collectionsCreated.indexOf(model.collectionName) === -1) {
      console.log('createCollection', model.collectionName)
      ...
      console.log('DB COLS'.bgRed, collections)
    ...
    if (ClassRef) {
      ...
      if (collectionName !== false) {
        ...
        this.timer = setTimeout(async () => {
          console.log('MAYOR OF SLAMTOWN'.bgMagenta)
          if (!isWaiting) {
            // must delete then() to prevent recursive loop
            delete ClassRef.then
  ...
  async createCollection(name, indexes) {
    ...
    console.log('AFTER EXISTS'.red)
    const collections = await conn.db.listCollections()
    console.log("COLLECTIONS".green, collections)
